...
public class CmsUserMngImpl implements CmsUserMng {
	...
	public Pagination getPage(Integer realnameStatus,Integer provinceId,Integer cityId,String username, String email, Integer siteId,
		...
		Pagination page = dao.getPage(realnameStatus,provinceId, cityId,username, email, siteId, groupId,
	...
	public Pagination getMembershipUser(String userName,String nickName,String mobile, Integer membership,int pageNo, int pageSize){
		return dao.getMembershipUser(userName, nickName, mobile, membership, pageNo, pageSize);
	}
	...
	public CmsUser registerMember( Integer provinceId,Integer cityId,String username, String email,
		...
		UnifiedUser unifiedUser = unifiedUserMng.save(username, email,
				password, ip);
		//获取区域信息
		Area province=areaMng.findById(provinceId);
		Area city=areaMng.findById(cityId);
	...
	public CmsUser registerAppMember(String username, String password,Integer groupId,String ip,CmsUserExt userExt,Map<String,String>attr) {
		UnifiedUser unifiedUser = unifiedUserMng.save(username, "",password, ip);
		CmsUser user = new CmsUser();
		user.forMember(unifiedUser);
		user.setAttr(attr);
		...
		user.setRealnameStatus(2);//2、待实名,APP注册用户为民用，需要实名认证
		user.setGroup(group);
		user.init();
		dao.save(user);
		cmsUserExtMng.save(userExt, user);
	...
	public CmsUser updateMember(Integer provinceId,Integer cityId,Integer id, String email, String password,
		...
		Area city=null;
		if(provinceId!=null){
			 province=areaMng.findById(provinceId);
		}
		
		if(cityId!=null){
			city =areaMng.findById(cityId);
		}
